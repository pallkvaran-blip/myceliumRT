...
| `traced-check.cjs` | Every traced map (pass ids to narrow it): sprites decode, open space runs colony→goal on the real mask, no food sealed off, and the rock art is CLIPPED to the pathClear channels (nothing solid lives there, so a sprite reaching in would be rock you grow through — the assertion moved from "no sprite overlaps" to "the clip bounds are the bounds dug" when the clip was added, because the overlap is a property of the tracer that any edited map loses) |
