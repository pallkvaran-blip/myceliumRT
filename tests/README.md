...
| `traced-check.cjs` | Every traced map (pass ids to narrow it): sprites decode, open space runs colony→goal on the real mask, no food sealed off, no sprite over a pathClear channel. That last one is gated on the map actually DIGGING channels — the 17 survival maps set `clearChannels: false` so their edge rock collides, and they take the "edge rock is solid" branch instead |
