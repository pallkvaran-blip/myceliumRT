...
| `0-survival-*.json` (17) | none | **the SURVIVAL pool** — every one carries `survival: true`, which puts it in the rotation AND makes its threats come from the LEVEL rather than from its own JSON. See "Survival's 17 authored maps" in `CLAUDE.md`; generated by `scripts/gen-survival-maps.mjs` + `scripts/place-survival-food.mjs` from the owner's exports |
