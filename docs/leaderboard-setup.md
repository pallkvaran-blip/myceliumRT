...
- **`logEvent` does not record the mode.** A real-time run and a turn-based run look identical in
  the `events` table, so the dashboard's numbers now mix the two games. Adding a `mode` field to
  `logEvent` (and a column to `events`) is the fix; nobody's done it yet.
