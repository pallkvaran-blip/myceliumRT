#!/usr/bin/env python3
"""Re-encode a STAGED asset tree in place. Used by BOTH public builds.

Called by scripts/make-itch-zip.mjs (high quality, no content dropped) and by
scripts/make-crazygames-zip.mjs (harder, to clear a 20 MB ceiling). Never run against
`assets/` itself — every path here is destructive and the repo's originals are the masters.

`--png 0` leaves PNGs alone, which is what the itch build wants: palette-quantising is a
real if small colour change, and itch has no size ceiling worth paying it for.
...
try:
    ...
    sys.exit("shrink-assets: Pillow is required (pip install Pillow)")
...
def main():
    ...
    ap.add_argument("--png", type=int, default=256, help="palette size for loose png art; 0 leaves png alone")
    args = ap.parse_args()

    root = os.path.abspath(args.stage)
    if os.path.basename(root) != "assets" or "/dist/" not in root.replace(os.sep, "/"):
        sys.exit(f"shrink-assets: refusing to touch {root} — this only runs on a staged dist/ copy")
